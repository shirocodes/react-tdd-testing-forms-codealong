...
test("'Your Selection' message updates when user selects toppings or size", () => {
  render(<App />);
  const addPepperoni = screen.getByRole("checkbox", { name: /add pepperoni/i });
  const selectSize = screen.getByLabelText(/select size/i);
  userEvent.click(addPepperoni);
...
test("'Contact Info' text box initially displays placeholder value", () => {
  render(<App />);
  expect(screen.getByPlaceholderText(/email address/i)).toBeInTheDocument();
});

test("page shows information the user types into the contact form", () => {
  render(<App />);
